...
dir <- paste0(getwd(), "/kallisto_quant_mouse_myoblast")
...
```{r}
files <- file.path(dir, c("SRR26779716.tabular", 
                          "SRR26779717.tabular",
                          "SRR26779718.tabular"))
                   
all(file.exists(files))

names(files) <- c("mb_tpm_1", "mb_tpm_2", "mb_tpm_3")
...
ggplot(marker_tpm, aes(x = SYMBOL, y = logmedian, fill = cell_type)) +
  ...
  theme_minimal() +
  labs(#title = "Log2 Median TPM Across 3 Bulk C2C12 Samples",
       x = "Gene Symbol",
       y = "Log2 Median TPM") +
  ylim(0, NA) +
...
ggsave(filename = "Log2 Median TPM 3 C2C12 Samples.png", width = 10, height = 12, dpi = 1200)
```
