---
title: "Top GEX Kallisto VSMC Mouse"
...
dir <- paste0(getwd(), "/kallisto_quant_mouse_vsmc")
...
```{r}
files <- file.path(dir, c("SRR14871083.tabular", 
                          "SRR14871084.tabular",
                          "SRR14871085.tabular",
                          "SRR14871086.tabular"))
...
names(files) <- c("vsmc_tpm_1", "vsmc_tpm_2", "vsmc_tpm_3", "vsmc_tpm_4")
...
```{r}
TxDb <- makeTxDbFromGFF(file = "gencode.vM35.annotation.gtf.gz")
...
ggplot(marker_tpm, aes(x = SYMBOL, y = logmedian, fill = cell_type)) +
  ...
  theme_minimal() +
  labs(title = "Log2 Median TPM Across 4 Mouse VSMC Samples",
       x = "Gene Symbol",
       y = "Log2 Median TPM") +
  ylim(0, NA) +
...
ggsave(filename = "Log2 Median TPM 4 Mouse VSMC Samples.png", width = 10, height = 12, dpi = 1200)
```
