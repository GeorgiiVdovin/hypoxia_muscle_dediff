...
```{r setup}
# To add a new package: renv::install("packageName") or renv::install("bioc::packageName"), then renv::snapshot(). Snapshots are always run in the console 
...
tgfbeta_diff_0_25 <- read.csv(paste0(results_path, "/TGFbeta_Diff_0.25%_5d_results.csv"))
```
...
# 4I Diff 0.25% TGFBi
```{r}
tgfbeta_diff_0_25 <- tgfbeta_diff_0_25 %>%
  mutate(
...
fit_tgfbeta_diff_0_25 <- glm(
  cbind(n_MT, n_Nuclei_no_MT) ~ Condition,
  family = binomial,         
  data   = tgfbeta_diff_0_25
)


anova_tgfbeta_diff_0_25 <- anova(fit_tgfbeta_diff_0_25, test = "LRT") 

# Get the estimated Diff.index per Condition & Tukey's all vs all contrasts
emm_tgfbeta_diff_0_25  <- emmeans(fit_tgfbeta_diff_0_25, "Condition", 
                             type = "response")       


pairs_tgfbeta_diff_0_25 <- pairs(emm_tgfbeta_diff_0_25, 
                              adjust = "tukey")
```
...
```{r}
# Results
anova_tgfbeta_diff_0_25
emm_tgfbeta_diff_0_25
pairs_tgfbeta_diff_0_25
```
...
# Differentiation index TFGBi + 10%FCS
```{r}
# Logistic regression with MT nuclei as success and nuclei outside MT as failure
